...
export async function generateSchedule(db: Db, start: Date, end: Date) {
  ...
  for (let cursor = startOfDay(start); cursor <= end; cursor = addDays(cursor, 1)) {
    for (const template of templates) {
      if (!matchesTemplate(template.recurrence, template.dayOfWeek, cursor)) continue;
...
function matchesTemplate(recurrence: string, dayOfWeek: number | null, date: Date) {
  if (dayOfWeek !== null && date.getDay() !== dayOfWeek) return false;
  if (recurrence === "weekly") return true;
  if (recurrence === "biweekly") return Math.floor(date.getTime() / 86400000 / 7) % 2 === 0;
  if (recurrence === "monthly") return date.getDate() <= 7;
...
export async function autoAssignInstance(db: Db, instanceId: string) {
  ...
  for (const serviceRole of instance.service.roles) {
    const existing = instance.assignments.find((assignment) => assignment.roleId === serviceRole.roleId);
    ...
    const sameDayFiltered = sorted.filter((member) => !usedToday.has(member.id));
    ...
    await db.assignment.upsert({
      ...
      create: { instanceId, roleId: serviceRole.roleId, memberId: assignee?.id ?? null }
    });
  }
}

export async function getRoster(db: Db, start: Date, end: Date, publicOnly = false) {
  return db.serviceInstance.findMany({
    ...
    orderBy: [{ date: "asc" }, { service: { time: "asc" } }]
  });
}
